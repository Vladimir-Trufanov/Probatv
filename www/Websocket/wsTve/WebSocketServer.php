...
class WebSocketServer 
{
  ...
  // Ключевое слово static в PHP используется для объявления функций, которые принадлежат 
  // самому классу, а не экземплярам класса. Это позволяет:
  // - получить доступ к функции без создания экземпляра класса. Например, вместо того 
  // чтобы писать: $object = new MyClass(); $object.someMethod(), можно написать: 
  // MyClass::someStaticMethod();
  // - реализовать функционал, не привязанный к конкретному экземпляру. Логику, которая 
  // может быть общей для нескольких экземпляров класса, можно извлечь и поместить 
  // внутрь статического метода;
  // - использовать статические методы для доступа к приватным статическим свойствам. 
  // Например, геттеры и сеттеры, которые нужны редко, но всё же бывают нужны.
  // Важно: статические методы не принадлежат объектам, они — часть класса, поэтому 
  // из них невозможно получить доступ к объекту через $this — внутри статического 
  // метода $this не существует. 
  // Синтаксис: чтобы объявить статическую функцию в PHP, перед её именем ставится 
  // модификатор static. Например: public static function test(). При обращении к 
  // статическим функциям используется имя класса и оператор ::, вместо операции доступа ->. 
  // Например: MyClass::test(). 
  ...
  private static function encode($payload, $type = 'text', $masked = false) 
  {
    ...
    {
      case 'text':
        // Первый байт указывает на FIN, Text-Frame (10000001):
        $frameHead[0] = 129;
        break;
      case 'close':
    ...
    // Устанавливаем маску и длину "полезной нагрузки" (используя 1, 3 или 9 байт)
    if ($payloadLength > 65535) 
    {
      // Представляем длину в двоичном виде, как строку. Дополняем нулями слева и 
      // разбивает её на части, как масиив строк по 8 символов
      $payloadLengthBin = str_split(sprintf('%064b', $payloadLength), 8);
      $frameHead[1] = ($masked === true) ? 255 : 127;
      for ($i = 0; $i < 8; $i++) 
      {
        // Преобразование строки из массива, представляющие двоичнык числа, в их десятичный эквивалент
        $frameHead[$i + 2] = bindec($payloadLengthBin[$i]);
      }
      // Если старший бит больше нуля (ДОЛЖЕН быть равен 0), то выдаем ошибку
      if ($frameHead[2] > 127) 
      {
    ...
    // Преобразовываем заголовок фрагмента в строку
    ...
    // Добавляем полезную нагрузку к фрагменту
  ...
  // **************************************************************************
  // *               Декодировать сообщение, полученное от клиента            *
  // **************************************************************************
  private static function decode($data) 
  {
    ...
    // Возвращаем ошибку, если получен немаскированный кадр
    if (!$isMasked) 
    {
    ...
    // Здесь мы должны проверить, является ли фрейм большим. socket_recv ограничен 
    // до 1024 байт, поэтому, если websocket-фрейм > 1024 байт, нам нужно подождать,
    // пока не будут переданы все данные.
  ...
  // **************************************************************************
  // *  «Рукопожатие», т.е. отправить заголовки согласно протоколу WebSocket  *
  // **************************************************************************
